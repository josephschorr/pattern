...
import sys
import types
from builtins import bytes, dict, filter, int, map, object, range, str, zip
...
from pattern.helpers import decode_string, encode_string
...
def train_topmine_ngrammer(
    documents, threshhold=1, max_ngramm_len=3, min_word_len=2, regexp="[.,!?;: ]", stopwords=None
):
    """
    ...
    for doc in documents:
        if isinstance(doc, str):
            splitted_docs.append(
                split_document_by_delimeters(
                    doc, regexp, min_word_len=min_word_len, stopwords=stopwords
                )
            )
...
class NGrammer(object):
            ...
            for doc_id in D:
                ...
                for i in A[doc_id][n - 1]:
                    if n == 2:
                        ...
                        if flag or flag2:
                            continue
                    ngram = "_".join([doc[i + j] for j in range(n - 1)])
                ...
                if not A[doc_id][n]:
                    ...
                    for i in A[doc_id][n]:
                        if i + 1 in A[doc_id][n]:
                            ngram = "_".join([doc[i + j] for j in range(n)])
    ...
    def _significanceScore(self, ngramm1, ngramm2):
        mu0 = float(self._phrase2freq.get(ngramm1, 0) * self._phrase2freq.get(ngramm2, 0))
        mu0 /= self._lengthInWords
        f12 = float(self._phrase2freq.get(ngramm1 + "_" + ngramm2, 0))
    ...
    def ngramm(self, token_list, threshhold, indexes=[]):
        ...
        for i in range(len(res) - 1):
            p1 = "_".join([token_list[w_i] for w_i in res[i]])
            p2 = "_".join([token_list[w_i] for w_i in res[i + 1]])
        ...
        while len(res) > 1:
            ...
            if Best > threshhold:
                ...
                if best_ind == 0:
                    ...
                    new_H = H[: best_ind - 1]
                    p1 = "_".join([token_list[w_i] for w_i in new_res[best_ind - 1]])
                    p2 = "_".join([token_list[w_i] for w_i in new_res[best_ind]])
                    new_H += [self._significanceScore(p1, p2)]
                if best_ind != len(new_res) - 1:
                    p1 = "_".join([token_list[w_i] for w_i in new_res[best_ind]])
                    p2 = "_".join([token_list[w_i] for w_i in new_res[best_ind + 1]])
                    new_H += [self._significanceScore(p1, p2)]
                new_H += H[best_ind + 2 :]
                H = new_H
                res = new_res
        ...
        for ngramm_ind in res:
            ngrammed_doc.append("_".join([token_list[x] for x in ngramm_ind]))
    ...
    def saveAsJson(self, filename, with_delimiters=False):
        to_save = {"lengthInWords": self._lengthInWords, "phrase2freq": self._phrase2freq}
        if with_delimiters:
...
class Model(object):
    def __init__(self, path="", classifier=None, known=set(), unknown=set()):
        ...
        try:
            from pattern.vector import Classifier, Perceptron
        except ImportError:
            sys.path.insert(0, os.path.join(MODULE, ".."))
            from vector import Classifier, Perceptron
        self._path = path
...
class TaggedString(str):
    ...
    def split(self, sep=TOKENS):
        ...
        if len(self) == 0:
            return []
        return [
            [[x.replace("&slash;", "/") for x in token.split("/")] for token in sentence.split(" ")]
...
DIACRITICS = diacritics = "àáâãäåąāæçćčςďèéêëēěęģìíîïīłįķļľņñňńйðòóôõöøþřšťùúûüůųýÿўžż"
...
RE_ABBR3 = re.compile(
    r"^[A-Z][%s]+.$" % ("|".join("bcdfghjklmnpqrstvwxz"))  # capital followed by consonants, "Mr."
)
...
# Common emoticons.
EMOTICONS = emoticons = {  # (facial expression, sentiment)-keys
    ("love", +1.00): set(("<3", "♥", "❤")),
...
# Common emoji.
EMOJI = emoji = {  # (facial expression, sentiment)-keys
    ("love", +1.00): set(("❤️", "💜", "💚", "💙", "💛", "💕")),
    ("grin", +1.00): set(("😀", "😄", "😃", "😆", "😅", "😂", "😁", "😻", "😍", "😈", "👌")),
    ("taunt", +0.75): set(("😛", "😝", "😜", "😋", "😇")),
    ("smile", +0.50): set(("😊", "😌", "😏", "😎", "☺", "👍")),
...
def find_tokens(
    ...
    while j < len(tokens):
        if tokens[j] in ("...", ".", "!", "?", EOS):
            while j < len(tokens) and (
                tokens[j] in ("...", ".", "!", "?", EOS) or tokens[j] in quotes
            ):
    ...
    sentences = [
        RE_EMOTICONS.sub(lambda m: m.group(1).replace(" ", "") + m.group(2), s) for s in sentences
    ]
    sentences = [
        RE_EMOJI.sub(lambda m: (m.group(1) or " ") + m.group(2) + (m.group(3) or " "), s)
        for s in sentences
    ]
...
def _suffix_rules(token, tag="NN"):
    ...
    if token.endswith("s") and not token.endswith(("is", "ous", "ss")):
        tag = "NNS"
    if (
        token.endswith(("able", "al", "ful", "ible", "ient", "ish", "ive", "less", "tic", "ous"))
        or "-" in token
    ):
        tag = "JJ"
    ...
    for i, chunk in enumerate(chunked):
        if chunk[2].endswith("PP") and chunk[-1] == "O":
            ...
            if i < len(chunked) - 1 and (
                chunked[i + 1][2].endswith(("NP", "PP")) or chunked[i + 1][1] in ("VBG", "VBN")
            ):
                chunk[-1] = "B-PNP"
                pp = True
    ...
    for sentence in parser.parse(s, chunks=True, lemmata=lemmata).split():
        for w in sentence:  # [token, tag, chunk, preposition, lemma]
            if w[2].startswith(("B", "O")):
                ...
                p = None
            if (
                w[1].startswith(("NNP", "DT"))
                and p
                and p[1].startswith("NNP")
                and p[0][0] != "@"
                and w[0][0] != "A"
            ):
...
def commandline(parse=Parser().parse):
    ...
    p.add_option(
        "-f", "--file", dest="file", action="store", help="text file to parse", metavar="FILE"
    )
    p.add_option(
        "-s",
        "--string",
        dest="string",
        action="store",
        help="text string to parse",
        metavar="STRING",
    )
    p.add_option(
        "-O", "--tokenize", dest="tokenize", action="store_true", help="tokenize the input"
    )
    ...
    p.add_option(
        ...
        dest="relations",
        action="store_true",
        help="find verb/predicate relations",
    )
...
# VERB TENSE:
INFINITIVE, PRESENT, PAST, FUTURE = INF, PRES, PST, FUT = "infinitive", "present", "past", "future"
...
for i, (tense, person, number, mood, aspect, negated, aliases) in TENSES.items():
    for a in aliases + (i,):
        TENSES_ID[i] = TENSES_ID[a] = TENSES_ID[(tense, person, number, mood, aspect, negated)] = i
    if number == SG:
...
class Sentiment(lazydict):
    ...
    def __call__(self, s, negation=True, ngrams=DEFAULT, **kwargs):
        ...
        return Score(
            polarity=avg(map(lambda w: (w[0], w[1]), a), weight),
...
class Spelling(lazydict):
    ...
    def suggest(self, w):
        ...
        if w.replace(".", "").isdigit():
            return [(w, 1.0)]  # 1.5
        candidates = (
            self._known([w]) or self._known(self._edit1(w)) or self._known(self._edit2(w)) or [w]
        )
